...
class VirtualUser(object):
    ...
    def login(self, rp_url):
        # connect to the vRP
        print("vuser: connecting to the vrp at", rp_url)
        rp_response = self.session.get(rp_url, allow_redirects=False)
        consent_url = rp_response.headers['Location']
        print("vuser: connecting to the consent/login page at ", consent_url)
        ...
        if consent_response.url != consent_url:
            ...
            print("vuser: logging in to ", login_page_url, " with given credentials")
            consent_response = self.session.post(login_page_url, data={
                'csrfmiddlewaretoken': csrf_token, 
                'next': nxt,
                'username': self.username, 
                'password': self.password}, headers={'referer': login_page_url})
            print("vuser: LOGIN PAGE URL: ", login_page_url)
            print("vuser: CONSENT PAGE URL: ", consent_response.url)
